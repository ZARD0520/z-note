...
export default function ChatContainer({ dict }: { dict: any }) {
  ...
  const { messages, isLoading, input, setInput, handleSubmit, stopGeneration, clearAllMessages } = useChat(menuValue as string, roleKey as string, defaultInput ?? '', chatActions)
  ...
  return (
    <div className="h-full flex flex-col justify-between">
      ...
      <SearchChat placeholder={'请输入你想问的问题'} roleKey={String(roleKey)} handleSelectRole={clickRoleItem} handleSubmit={handleSubmit} handleStop={stopGeneration} setInput={setInput} isLoading={isLoading} input={input} className='mt-2 justify-end'></SearchChat>
    </div>
  )
}