...
const suffixInput = (valueKey: string = '1', onSelect?: onSelectRoleType, onSearch?: onSearchType, loading?: boolean) => (
  <div className='flex items-center ml-2'>
    ...
    <div className="ml-auto h-10 w-10 m-2 rounded-sm cursor-pointer hover:bg-primary-background flex items-center justify-center" onClick={onSearch}>
      {loading ? <i className="iconfont icon-stop-circle text-primary-disabled"></i> : <i className="iconfont icon-fasong text-primary-disabled"></i>}
    </div>
  </div>
)
...
const zSearchInput: React.FC<InputComponentProps> = ({
  ...
  return (
    <ZInput type={type} rows={rows} onChange={onChange} value={value} isLoading={isLoading} onSearch={onSearch} placeholder={placeholder} suffix={() => suffixInput(roleKey, onSelectRole, onSearch, isLoading)} className={"border-2 border-primary-border " + className} {...restProps} />
  );
};
