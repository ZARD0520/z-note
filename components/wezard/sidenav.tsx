...
const SideNav: React.FC<SideNavProps> = ({ contentList, isMobile, currentPage, goToPage }) => {
  ...
  const showContent = !isMobile ? (
    <nav className="absolute w-20 right-2 top-2 z-50 mt-16 mr-12 flex flex-col border-l-2">
      {contentList.map((item, index) => (
        <p
          ...
          className={`w-full pl-8 mb-8 text-gray-200 cursor-pointer ${currentPage === index ? 'border-l-4' : ''}`}
          key={item.title}
        >
          {item.title}
        </p>
    ...
    <div className="absolute right-2 top-2" aria-label={dict.chat.tools.more}>
      ...
      >
        <div
          className={`cursor-pointer absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 transition-all duration-300 ${isOpen ? 'opacity-0' : 'opacity-100'}`}
        >
        ...
        <div className="fixed inset-0 w-screen h-screen bg-black bg-opacity-50 z-40">
          <div className="w-full">
            <nav className="pt-4 mr-4 pl-4 h-full flex flex-col">
              {contentList.map((item, index) => (
                <p
                  onClick={() => handleMobileGo(index)}
                  className="mb-8 text-gray-200 cursor-pointer"
                  key={item.title}
                >
