...
export async function getMediaList(params: getListParams) {
  ...
  return list?.filter(
    (item, index) =>
      index >= (params.current - 1) * params.size && index < params.current * params.size
  )
}
...
async function scanMediaType(type: keyof typeof MEDIA_CONFIG, metadata: Record<string, MediaFile>) {
